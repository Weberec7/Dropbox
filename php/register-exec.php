<?php
	...
	$pages[9] = $_POST['services']*29;  //services  (29)
	$password = clean($_POST['password']);
	$cpassword = clean($_POST['cpassword']);
	$webname = clean($_POST['webname']);
	$agree = $_POST['agree'];
	$wverify = clean($_POST['wverify']);
	$captcha = file_get_contents("http://captchator.com/captcha/check_answer/".$_SESSION['My_ID']."/".$wverify);
	//Input Validations
	if(!$captcha){
		$errmsg_arr[] = 'Word Verification was incorrect';
		$errflag = true;
	}
	if(!$agree){
		$errmsg_arr[] = 'Please Read and agree to the terms and conditions';
		$errflag = true;
	}
	if($fname == '') {
